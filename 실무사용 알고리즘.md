...
#### 데이터 색인 비교
  ```ruby
	std::vector<CAchievementData> _vecAchievementRows;
	std::unordered_map<INT32, size_t> _mapIndexToPos;
  ```
vs
  ```ruby
	std::vector<CAchievementData> _vecAchievementRows;
	std::unordered_map<INT32, CAchievementData*> _mapIndexToPos;
  ```

백터의 배열위치를 기억하는 방식 vs 포인터를 찾는 방식으로 비교를 해봤다.\
결론은 후자가 더 빨랐는데, 이유는 unordered_map.find가 무겁기 때문에 at을 사용한 호출을 하여도 느리다 였다\
하지만, 장점으로는 사용자의 실수를 줄일 수 있고 허상포인터(Dangling Pointer)가 생기는걸 막을 수 있다.
